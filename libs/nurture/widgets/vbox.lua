...
function VBox:new(N, options)
    ...
    self.vertAlign = options.vertAlign or "center"
    self.horizAlign = options.horizAlign or "center"

    self.stackVertAlign = options.stackVertAlign or "center"
    self.stackHorizAlign = options.stackHorizAlign or "center"
...
---@diagnostic disable-next-line: duplicate-set-field
function VBox:onDrag(x, y, dx, dy)
    for _, childUUID in ipairs(self.childrenUUIDs) do
        local child = self.nurture:getFromUUID(childUUID)
...
---@diagnostic disable-next-line: duplicate-set-field
function VBox:onDragEnd(x, y, button)
    for _, childUUID in ipairs(self.childrenUUIDs) do
        local child = self.nurture:getFromUUID(childUUID)
